...
"""
        ...
        # Generate with the full evidence first. If Gemini rejects the
        # request because the context is too large or transiently fails,
        # retry with progressively smaller evidence instead of exposing a
        # useless "final answer could not be generated" message to the user.
        attempts = [
            (evidence[:12], min(self.max_output_tokens, 4096)),
            (evidence[:8], 2048),
            (evidence[:5], 1536),
            (evidence[:3], 1024),
        ]
        ...
        for attempt_evidence, output_tokens in attempts:
            compact_prompt = f"""
SOURCE TYPE:
...
"""
            ...
            try:
                ...
                last_error = e
                print(
                    f"Final answer generation attempt failed "
                    f"(evidence={len(attempt_evidence)}, "
                    f"tokens={output_tokens}): {e}"
                )

        # Last-resort grounded response. Never tell the user that the
        # system failed while useful evidence is already available.
        first = evidence[0]
        print(f"Final answer generation failed after retries: {last_error}")

        return (
            f"According to {first['source']}, the retrieved document evidence "
            "contains the following information relevant to your question:\n\n"
            f"{first['content']}"
        )
