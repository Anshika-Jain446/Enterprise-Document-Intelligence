...
class PostgreSQLStore:
    ...
    def save_document(
        ...
        try:
            ...
            if not row:
                raise RuntimeError(
                    "PostgreSQL did not return the new document ID."
                )

            document_id = int(row[0])

            # DO NOT COMMIT YET.
            # Chunks will be written in the same transaction.
    ...
    def save_chunks(
        ...
        try:
            ...
            if chunk_count <= 0:
                raise RuntimeError(
                    "Document was inserted but no chunks were saved."
                )
            ...
            if not document_exists:
                raise RuntimeError(
                    "Document was inserted but could not be verified."
                )
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        # Analytics is intentionally NOT available
        # to normal users.
...
def documents_page():
    ...
    if uploaded_files:
        ...
        if st.button(
            ...
            for uploaded_file in uploaded_files:
                ...
                try:
                    ...
                    with st.status(
                        ...
                        st.write(
                            "💾 Saving document and chunks to PostgreSQL..."
                        )
                        ...
                        # Final verification using a fresh
                        # database read.
                        ...
                        if not verified:
                            ...
                            raise RuntimeError(
                                "PostgreSQL transaction completed "
                                "but the document could not be found "
                                "after commit."
                            )
            ...
            # ------------------------------------------------
            # IMPORTANT:
            # Refresh the entire Streamlit page so the
            # document list is fetched again from PostgreSQL.
            # ------------------------------------------------
    ...
    search_value = st.text_input(
        ...
        placeholder="Example: attention, NIPS, report...",
        key="document_search_input",
    )

    st.session_state.document_search = search_value
    ...
    if not search_value:
        ...
        existing_ids = {
            d["id"]
            for d in all_user_documents
        }

    st.session_state.selected_document_ids = [
        x
        for x in st.session_state.selected_document_ids
    ...
    if documents:
        ...
        with col1:
            ...
            if st.button(
                ...
                for document_id in visible_ids:
                    ...
                    if (
                        document_id
                        not in st.session_state.selected_document_ids
                    ):
        ...
        if selected_count:
            ...
            st.info(
                ...
                "Chat will search across all your documents."
            )
    ...
    if not documents:
        ...
        if search_value:
            ...
            st.warning(
                f'No documents found for "{search_value}".'
            )
