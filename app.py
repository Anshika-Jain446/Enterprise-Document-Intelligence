...
def retrieve_documents(
    ...
    if retriever is not None and selected_sources:
        try:
            results = retriever.retrieve(
                ...
                chunk_types=active_chunk_types, top_k=max(top_k, 10),
            )
            if results:
                print(
                    "ALL-DOCUMENT VECTOR RETRIEVAL:", query,
                    "| DOCUMENTS IN SCOPE:", len(selected_sources),
    ...
    # PostgreSQL remains the authoritative user-scoped fallback.
    try:
        results = st.session_state.db.search_chunks(
            user_id=user["id"], query=query,
            selected_document_ids=None, chunk_types=active_chunk_types,
            top_k=top_k,
        )
        if results:
            print(
                "ALL-DOCUMENT POSTGRES RETRIEVAL:", query,
                "| RETRIEVED CHUNKS:", len(results),
            )
            return results
    except Exception as exc:
