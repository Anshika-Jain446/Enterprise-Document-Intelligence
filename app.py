...
POSTGRES_PORT = int(
    config_value("POSTGRES_PORT", 5432)
    or 5432
)
...
CHUNK_SIZE = int(
    config_value("CHUNK_SIZE", 1000)
    or 1000
)

CHUNK_OVERLAP = int(
    config_value("CHUNK_OVERLAP", 100)
    or 100
)
...
class PostgreSQLStore:
    ...
    #
    # IMPORTANT:
    # save_document_with_chunks() saves the document and ALL
    # chunks using ONE connection and ONE transaction.
    # ========================================================
    ...
    def save_document_with_chunks(
        ...
        try:
            ...
            # Remove previous version of same filename
            cursor.execute(
                """
            ...
            # ------------------------------------------------
            # Verify document exists BEFORE inserting chunks
            # ------------------------------------------------
            ...
            if not verified:
                raise RuntimeError(
                    "Document was inserted but could not be "
                    "verified inside the PostgreSQL transaction."
                )

            # ------------------------------------------------
            # Insert chunks using SAME connection
            # ------------------------------------------------
            ...
            if not verification:
                raise RuntimeError(
                    "Document could not be verified after "
                    "chunk insertion."
                )

            if int(verification[2]) != len(chunks):
                raise RuntimeError(
...
def initialize_session_state():
    ...
    defaults = {
        ...
        # Prevent upload widgets from getting stale
        "documents_refresh": 0,
    }
...
def retrieve_documents(query):
    ...
    return (
        st.session_state.db.search_chunks(
            ...
            # If documents are selected, search ONLY them.
            selected_document_ids=(
                st.session_state.selected_document_ids
                or None
            ),
...
def chat_page():
    ...
    if selected_ids:
        ...
        st.info(
            ...
            "Go to Documents to change the selection."
        )
...
def documents_page():
    ...
    if uploaded_files:
        ...
        if st.button(
            ...
            for uploaded_file in uploaded_files:
                ...
                try:
                    ...
                    with st.status(
                        ...
                        st.write(
                            f"✅ PostgreSQL document ID: "
                            f"{document_id}"
                        )

                        st.write(
                            "✅ Document and chunks committed."
                        )
            ...
            if upload_success:
                ...
                st.session_state.documents_refresh += 1
    ...
    with search_col:
        ...
        document_search = st.text_input(
            ...
            placeholder="Example: attention, report, invoice...",
    ...
    documents = (
        st.session_state.db.get_documents(
            user_id=user["id"],
            is_admin=is_admin,
            search_term=st.session_state.document_search,
        )
    )
    ...
    # We should not remove selected docs just because
    # the search box hides them.
    all_documents = (
        st.session_state.db.get_documents(
            user_id=user["id"],
            is_admin=is_admin,
    ...
    st.session_state.selected_document_ids = [
        ...
        if int(doc_id) in all_document_ids
    ]
    ...
    if selected_count:
        ...
        st.success(
            f"{selected_count} document(s) selected. "
            "Chat will search ONLY these documents."
        )

        if st.button(
            "🌐 Search all my documents instead",
            use_container_width=True,
        ):
        ...
        with st.container(
            ...
            with col1:
                ...
                st.caption(
                    ...
                    f" • Chunks: "
                    f"{document.get('metadata', {}).get('chunks', '?')}"
                )
