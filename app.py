...
class PostgreSQLStore:
    ...
    def save_document(
        ...
        try:
            ...
            # Replace old document with same filename
...
def initialize_session_state():
    ...
    defaults = {
        ...
        # Selected documents for retrieval
        "selected_document_ids": [],
...
The application also supports configuration values
from `config.py`.
"""
        )
...
def retrieve_documents(query):
    ...
    return (
        st.session_state.db.search_chunks(
            ...
            filename_filter=(
                st.session_state.document_search.strip()
                or None
            ),
        )
    )
...
def render_document_selector():
    ...
    st.caption(
        "Leave everything unselected to search "
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        # IMPORTANT:
        # Analytics is deliberately NOT shown to normal users.
...
def chat_page():
    ...
    # --------------------------------------------------------
    # Document search / selection
    # --------------------------------------------------------

    with st.expander(
        "🎯 Search a particular document",
        expanded=False,
    ):
    ...
    elif st.session_state.document_search.strip():
...
def documents_page():
    ...
    if uploaded_files:
        ...
        if st.button(
            ...
            for uploaded_file in uploaded_files:
                ...
                try:
                    ...
                    with st.status(
                        ...
                        saved_document = (
                            st.session_state.db.get_document(
                                document_id,
                                user["id"],
                            )
                        )
                        ...
                        if not saved_document:
                            ...
                            raise RuntimeError(
                                "Document was saved but could not "
                                "be verified in PostgreSQL."
                            )
            ...
            # ------------------------------------------------
            # IMPORTANT FIX:
            # Force Streamlit to rerun after upload.
            # This makes the new document appear immediately.
            # ------------------------------------------------
            ...
            if successful_uploads > 0:
                ...
                st.toast(
                    f"{successful_uploads} document(s) uploaded successfully."
                )
    ...
    if document_filter.strip():
        ...
        filtered_documents = [
            ...
            if document_filter.strip().lower()
    ...
    st.caption(
        f"Showing {len(filtered_documents)} of "
...
# Normal users do not see it in navigation.
# ============================================================
