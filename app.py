...
class PostgreSQLStore:
    ...
    def connect(self):
        ...
        if self.connection is not None:
            ...
            try:
                ...
                if self.connection.closed == 0:
                    ...
                    if self.connection.get_transaction_status() != 0:
        ...
        # DATABASE_URL is preferred on Streamlit Cloud
        if DATABASE_URL:
    ...
    def initialize(self):
        ...
        try:
            ...
            #
            # IMPORTANT:
            # Do NOT depend only on CREATE TABLE IF NOT EXISTS.
            #
            # Your existing database may have a users table
            # created by an older version of the application.
            #
            # PostgreSQL foreign keys require the referenced
            # column to have PRIMARY KEY or UNIQUE protection.
            #
            ...
            # ------------------------------------------------
            # RECONNECT CLEANLY AFTER COMMIT
            # ------------------------------------------------

            cursor.close()

            # ------------------------------------------------
            # REPAIR USERS.ID UNIQUENESS
            # ------------------------------------------------
            ...
            #
            # If an old PostgreSQL installation rejects the
            # expression index, the application still starts.
            #
            ...
            migrations = [
                ...
                """
                ALTER TABLE document_chunks
                ADD COLUMN IF NOT EXISTS chunk_data JSONB
                DEFAULT '{}'::jsonb;
                """,
            ]
            ...
            # VERY IMPORTANT:
            # Never leave psycopg2 inside an aborted transaction.
            conn.rollback()
    ...
    def save_document(
        ...
        try:
            ...
            # Delete old document with same filename.
...
if st.session_state.db is None:
    ...
    try:
        ...
        # Throw away poisoned connection.
        try:
        ...
        if st.button(
            ...
            if not username or not password:
                ...
                try:
                    ...
                    # Recover connection if anything
                    # unexpected happens during login.
                    try:
                        st.session_state.db.rollback()
                    except Exception:
                        pass
...
# ============================================================
#
# WEB SEARCH IS STILL ENABLED.
#
# GeminiModel is responsible for the actual implementation.
# No web-search API key or URL is removed from app.py.
# ============================================================
