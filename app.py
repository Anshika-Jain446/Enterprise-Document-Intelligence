...
class PostgreSQLStore:
    ...
    def save_document_with_chunks(
        ...
        """
        IMPORTANT:
        Document and all chunks are inserted in ONE transaction.

        This prevents:
        document_id exists temporarily
        -> transaction gets rolled back
        -> chunk insert fails with FK violation.
        """
        ...
        try:
            ...
            # Remove old copy of same filename for this user.
            cursor.execute(
                """
            ...
            # Make absolutely sure document exists
            ...
            if not verified:
                raise RuntimeError(
                    "Document was inserted but could not "
                    "be verified inside the transaction."
                )

            # Insert chunks using the SAME cursor/transaction.
            ...
            if chunk_count <= 0:
                raise RuntimeError(
                    "Document was inserted but no chunks "
                    "were saved."
                )

            # ONE COMMIT for document + chunks.
            conn.commit()
            ...
            # If anything fails, BOTH document and chunks
            # are rolled back.
            conn.rollback()
            raise
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        # Analytics removed from user interface.
        pages = [
            "💬 Chat",
            "📚 Documents",
        ]
...
def documents_page():
    ...
    if uploaded_files:
        ...
        if st.button(
            ...
            # Force Streamlit to reload the document list.
            if upload_success:
    ...
    current_selected = [
        ...
        if doc_id in document_options.values()
    ]

    selected_labels = [
        label
        for label, doc_id in document_options.items()
        if doc_id in current_selected
    ]
    ...
    selected_labels = st.multiselect(
        ...
        help=(
            "If you select documents here, Chat will "
            "search only those documents. Leave empty "
            "to search all of your documents."
        ),
    )
