...
def retrieve_documents(
    ...
    # --------------------------------------------------------
    # No selection means ALL documents in the current user's scope.
    selected_sources = []
    try:
    ...
    # --------------------------------------------------------
    # PostgreSQL is the authoritative per-user document index.
    # This is important for queries such as "What is an encoder?":
    # FAISS may return semantically nearby chunks from another document,
    # while PostgreSQL can directly locate the exact keyword in the
    # uploaded document (for example, the Attention paper).
    try:
        results = st.session_state.db.search_chunks(
            user_id=user["id"],
            query=query,
    ...
    # --------------------------------------------------------
    # Explicitly pass the user's document filenames to FAISS. Never let
    # an empty selection become an unrestricted global-index search.
    retriever = st.session_state.multi_document_retriever
...
def perform_rag(
    ...
    if llm is None:
        ...
        return (
            ...
            "Configure GOOGLE_API_KEY to enable "
    ...
    # --------------------------------------------------------
    # Always give uploaded documents the first chance to answer.
    # This prevents the planner/evidence evaluator from turning a
    # valid local match such as "encoder" into a false web fallback.
    local_preflight = retrieve_documents(query)
    if local_preflight and _has_local_keyword_evidence(query, local_preflight):
        try:
            answer = llm.generate_answer(
                query=query,
                chunks=local_preflight,
                source_type="uploaded documents",
            )
            return answer, local_preflight, "documents"
        except Exception as exc:
    ...
    if action == "vector_search":
        ...
        if local_results:
            ...
            if _has_local_keyword_evidence(query, local_results):
                evaluation = {
                    "sufficient": True,
                    "confidence": 1.0,
                }
